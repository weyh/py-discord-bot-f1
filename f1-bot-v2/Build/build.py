...
def GenVersionFileForWin(version):
    return str('VSVersionInfo(\n' +
               ...
               "StringStruct(u'InternalName', u''),StringStruct(u'LegalCopyright', u''),StringStruct(u'OriginalFilename', u'f1_bot_v2.py')," +
               f"StringStruct(u'ProductName', u'Discord Formula 1, Formula 2 Bot ({architecture})')," +
               f"StringStruct(u'ProductVersion', u'{version.replace('v', '')}')])]),\n" +
               "VarFileInfo([VarStruct(u'Translation', [1033, 1200])])])")

def read_version():
    all = ""
    with open("f1_bot_v2.py", 'r') as file:
        all = file.read()
